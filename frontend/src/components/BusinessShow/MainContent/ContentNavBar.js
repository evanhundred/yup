...
const ContentNavBar = ({ business, currentUser, handleWriteReview }) => {
  ...
  const savedStateData = fetchedUser ? determineIfSaved() : defaultSavedBizState;

  const [showShareModal, setShowShareModal] = useState(false);
  const [businessIsSaved, setBusinessIsSaved] = useState(savedStateData.businessIsSaved);
  ...
  const ShareModal = () => {
    return (
      ...
      >
        <div className='share-modal-overlay' onClick={(e) => handleCloseModal(e)} />
        <div className='share-modal-box'>
          <div className='share-modal-content'>
            ...
            <div className='share-modal-line-2'>
              <a target='_blank' rel='noopener noreferrer' href={`https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fyup.evanryan.dev%2Fbusinesses%2F${business.id}&amp;src=sdkpreparse`} className='fb-xfbml-parse-ignore'>
                <div className='share-fb'>
                  <div className='fb-logo-container'>f</div>
                  <h3>Share on Facebook</h3>
                </div>
              </a>
              <a rel='noopener noreferrer' target='_blank' href={`https://twitter.com/intent/tweet?text=Check out this amazing business on Yup.&url=https://yup.evanryan.dev/businesses/${business.id}`} className='twitter-share-button' data-show-count='false'>
            ...
            <div className='share-modal-line-3'>
              <div className='border-holder'>
                <div className='link-icon-container' onClick={(e) => copyToClipboard(e)}>
                  <img src={CopyIcon} alt='copy this link' />
                  {copySuccess === 'Copied!' && <CopySuccessDiv />}
                </div>
                <div className='share-link-input-container'>
                  <input ref={textAreaRef} className='share-link' defaultValue={`https://yup.evanryan.dev/businesses/${business.id}`} />
                </div>
              </div>
            </div>
          </div>
  ...
  const handleSaveClick = async () => {
    if (currentUser) {
      const savedDiv = document.querySelector('div.save-bookmark-button.button-container');
      ...
      if (!businessIsSaved) {
        ...
        const res = await dispatch(deleteSavedBusiness(savedBizId || savedStateData.savedBizId));
  ...
  const savedTextString = () => (businessIsSaved || savedStateData.businessIsSaved ? 'Saved' : 'Save');
  ...
  const currentUserIsOwner = currentUser ? business.owns && business.owns.some((own) => own.ownerId === currentUser.id) : false;

  return (
    <div className='content-nav-bar-container'>
      <div className='write-review-button container' onClick={(e) => handleWriteReview(e)}>
      ...
      <div className='share-button container button-container' onClick={(e) => handleShareClick(e)}>
      ...
      <div className={`save-bookmark-button container button-container ${businessIsSaved || savedStateData.businessIsSaved ? 'saved' : 'unsaved'}`} onClick={handleSaveClick}>
